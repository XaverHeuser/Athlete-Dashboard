...
def load_stats_history() -> pd.DataFrame:
    ...
    query = f'SELECT * FROM {table_fqn} ORDER BY snapshot_date'
    return client.query(query).to_dataframe()
...
def load_activities() -> pd.DataFrame:
    ...
    query = f'SELECT * FROM {table_fqn} ORDER BY start_date_local DESC'
    return client.query(query).to_dataframe()
...
def load_time_series(
    ...
    query = f"""
        ...
        GROUP BY period
        ORDER BY period
    """  # nosec B608: table/time_col/metric are allowlisted; values parameterized
