...
from dashboard.ui.constants import PAGE_SIZE, KPI_ICONS
...
# Distance
filtered = filtered[
    (filtered["distance_km"] >= min_dist)
    & (filtered["distance_km"] <= max_dist)
]
...
filtered = filtered.sort_values(
    "activity_date_local", ascending=False
        ...
        with cols[0]:
            ...
            st.markdown(
                ...
                <div style="margin-top: 8px; line-height: 1.6;">
                    <div>{KPI_ICONS['distance']} <strong>Distance:</strong> {row['distance_km']:.1f} km</div>
                    <div>{KPI_ICONS['speed']} <strong>Avg speed:</strong> {row['avg_speed_kph']:.1f} km/h</div>
