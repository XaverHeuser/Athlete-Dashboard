...
preview['avg_heartrate'] = preview['avg_heartrate'].map(
...
if selected:
    ...
    st.caption(
        f'Date: {pd.to_datetime(act["start_date_local"]).date()} | Distance: {act["distance_km"]:.1f} km'
    )
    ...
    col4.metric(
        'Avg HR', f'{act["avg_heartrate"]:.0f} bpm' if act['avg_heartrate'] else '–'
    )
