...
def get_selected_activity_id() -> str | None:
    val = st.query_params.get("activity_id")
...
with row1_col3:
    if year_filter != "All":
        month_options = ["All"] + sorted(
            df_activities.loc[df_activities["activity_year"] == year_filter, "activity_month"]
            .dropna()
            .unique()
            .tolist()
        )
...
filtered = filtered[(filtered["distance_km"] >= min_dist) & (filtered["distance_km"] <= max_dist)]
filtered = filtered[
    (filtered["moving_time_s"] >= min_time * 60) & (filtered["moving_time_s"] <= max_time * 60)
]

filtered = filtered.sort_values("activity_date_local", ascending=False).reset_index(drop=True)

# =========================
# Selected activity (from URL)
# =========================
selected_activity_id = get_selected_activity_id()
selected_activity_id_int: int | None = None
if selected_activity_id:
    try:
        selected_activity_id_int = int(selected_activity_id)
    except ValueError:
        clear_selected_activity_id()
        selected_activity_id_int = None

# =========================
# Activity cards + INLINE details
# =========================
st.divider()
...
for _, row in visible.iterrows():
    activity_id_int = int(row["activity_id"])
    ...
    with st.container():
        ...
        with cols[0]:
            ...
            with btn1:
                if selected_activity_id_int != activity_id_int:
                    if st.button("View details", key=f"open_{activity_id_int}"):
                        set_selected_activity_id(str(activity_id_int))
                        st.rerun()
                else:
                    if st.button("Close", key=f"close_{activity_id_int}"):
                        clear_selected_activity_id()
                        st.rerun()
        ...
        if selected_activity_id_int == activity_id_int:
            with st.container(border=True):
                try:
                    df_streams = load_activity_streams(activity_id_int)
                except Exception as e:
