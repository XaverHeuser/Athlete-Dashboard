...
from ui.viz_helper_functions import format_seconds_to_hhmmss, format_pace_min_per_km
...
def render_activity_details(*, activity_row: pd.Series, df_streams: pd.DataFrame) -> None:
    """Render detail panel for a selected activity."""
    st.subheader(f"Details – {activity_row.get('activity_name', 'Activity')}")

    # ---- High-level KPIs ----
    moving_time_str = format_seconds_to_hhmmss(int(activity_row.get("moving_time_s", 0) or 0))
    pace_str = format_pace_min_per_km(activity_row.get("avg_pace_min_per_km"))

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    with c1:
        st.metric("Distance", f"{float(activity_row.get('distance_km', 0.0) or 0.0):.2f} km")
    with c2:
        st.metric("Moving time", f"{moving_time_str} h")
    with c3:
        st.metric("Avg pace", f"{pace_str} min/km")
    with c4:
        st.metric("Avg HR", f"{float(activity_row.get('avg_heartrate', 0.0) or 0.0):.0f} bpm")
    with c5:
        st.metric("Avg speed", f"{float(activity_row.get('avg_speed_kph', 0.0) or 0.0):.2f} km/h")
    with c6:
        st.metric("Elev gain", f"{float(activity_row.get('elevation_gain_m', 0.0) or 0.0):.0f} m")
        ...
        def line_chart(y_col: str, title: str, y_title: str) -> None:
            ...
            chart = (
                ...
                .encode(
                    ...
                    tooltip=[alt.Tooltip(x_col, title=x_mode), alt.Tooltip(y_col, title=y_title)],
                )
    ...
    with map_tab:
        ...
        if not has_latlng:
            ...
            st.caption("Lat/Lng stream points available. Next: render pydeck track map here.")
